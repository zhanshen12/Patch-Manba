"""
Runtime batch construction for recursive rollout training.
"""
...
try:
    ...
    raise ImportError(
        "The training module requires `patch_dataset.EPS` from the companion dataset builder."
    ) from exc

from .metrics import restore_pred_lonlat_torch
...
def encode_raw_points_to_model_input_torch(
    ...
    """
    Encode raw 5D trajectory points into 6D model-input features.
    """
    if points5.numel() == 0:
...
def collect_input_patches_as_model_input_torch(
    ...
    window_start_ts: float | torch.Tensor,
    input_patch_num: int,
    patch_minutes: int,
    restore_info: torch.Tensor,
):
    """
    Rebuild patch-wise model inputs from raw 6D points during rollout.
    """
    ...
    for patch_id in range(input_patch_num):
        ...
        patch_id_list.append(torch.full((patch_points.size(0),), patch_id + 1, dtype=torch.long, device=device))
    ...
    return torch.cat(feat_list, dim=0), torch.cat(patch_id_list, dim=0), patch_mask
...
def build_runtime_singleview_batch_from_tensor_points(
    ...
    """
    Build one runtime batch for recursive single-view training.
    """
    history_sec = int(input_patch_num * patch_minutes * 60)
    bsz = observed_points6.size(0)
    ...
    for i in range(bsz):
        obs = observed_points6[i][observed_points6_mask[i] > 0.5]
        if generated_points6 is not None and generated_points6_mask is not None and generated_points6.size(1) > 0:
            gen = generated_points6[i][generated_points6_mask[i] > 0.5]
            points = torch.cat([obs, gen], dim=0)
        else:
            points = obs

        cut_time_ts = current_cut_times[i]
        ...
        model_input_i, patch_index_i, patch_mask_i = collect_input_patches_as_model_input_torch(
            ...
            restore_info=restore_info_batch[i],
        )
    ...
    model_input = torch.full((bsz, max_seq_len, 6), float(pad_value), dtype=dtype, device=device)
    sequence_mask = torch.zeros((bsz, max_seq_len), dtype=dtype, device=device)
    patch_index = torch.zeros((bsz, max_seq_len), dtype=torch.long, device=device)
    patch_mask = torch.stack(per_sample_patch_mask, dim=0) if per_sample_patch_mask else torch.zeros((bsz, input_patch_num), dtype=dtype, device=device)

    for i in range(bsz):
        seq_len = per_sample_model_input[i].size(0)
        if seq_len <= 0:
            continue
        model_input[i, :seq_len] = per_sample_model_input[i]
        sequence_mask[i, :seq_len] = 1.0
        patch_index[i, :seq_len] = per_sample_patch_index[i]
...
def build_runtime_multiview_batch_from_tensor_points(
    ...
    """
    Build one runtime batch for recursive multi-window fusion training.
    """
    out = {"restore_info": restore_info_batch}
    for view in view_settings:
        ...
        out[f"{name}__model_input"] = single_batch["model_input"]
        out[f"{name}__sequence_mask"] = single_batch["sequence_mask"]
        out[f"{name}__patch_index"] = single_batch["patch_index"]
        out[f"{name}__patch_mask"] = single_batch["patch_mask"]
    return out


def pred5_to_raw_point6_torch(pred5: torch.Tensor, restore_info: torch.Tensor, target_ts: torch.Tensor) -> torch.Tensor:
    """
    Convert a batch of 5D model predictions into 6D raw-point feedback tensors.
    """
    pred5 = pred5.float()
    restore_info = restore_info.float()
    ...
    return torch.stack(
        [lonlat_raw[:, 0], lonlat_raw[:, 1], sog_raw, cog_raw, target_ts, interp_flag],
        dim=1,
    )
...
def rollout_forward(
    ...
    """
    Run closed-loop recursive rollout over a batch.
    """
    ...
    _, num_steps, _ = future_model_labels.shape
    ...
    pred_steps = torch.stack(pred_steps_list, dim=1) if pred_steps_list else torch.zeros_like(future_model_labels)
...
def estimate_runtime_patch_len_from_rollout_samples(
    ...
    """
    Estimate a safe runtime patch length for recursive rollout training.
    """
    history_sec = int(input_patch_num * patch_minutes * 60)
    patch_sec = int(patch_minutes * 60)
    future_step_sec = int(future_step_minutes * 60)
    ...
    for sample in samples:
        ...
        for step_idx in range(len(future_points6)):
            ...
            for patch_id in range(input_patch_num):
                left = window_start + patch_id * patch_sec
                right = left + patch_sec
                if patch_id < input_patch_num - 1:
