"""Model configuration builders for modular Patch-Mamba training."""
...
def make_model_cfg(
    ...
    **kwargs,
    ...
    """Build the configuration dictionary for the single-view predictor.

    Parameters
    ----------
    batch_data:
        Packed training batch dictionary used to infer feature and target sizes.
    auto_patch_len:
        Maximum number of points observed in one patch.
    device:
    ...
    **kwargs:
        Additional model hyperparameters such as graph-attention settings and
        dropout.

    Returns
    -------
    dict
        Configuration dictionary consumed by ``PatchTTCN_Mamba_TrajPred``.
    """
...
def make_multiwindow_model_cfg(
    branch_batch_data: Dict[str, Dict[str, object]],
    device,
    view_settings: Sequence[Dict[str, object]],
    branch_auto_patch_len: Dict[str, int],
    **kwargs,
) -> Dict[str, object]:
    """Build the configuration dictionary for the hybrid multi-window predictor."""
    branch_cfgs = []
